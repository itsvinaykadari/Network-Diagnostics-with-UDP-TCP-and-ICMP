"""
Network Diagnostics: TCP Pinger Client

PROBLEM STATEMENT:
    TCP-based connectivity testing provides reliable, ordered message delivery.
    This utility measures network performance over TCP connections, accounting for
    connection establishment overhead and providing accurate latency measurements.

DESCRIPTION:
    This module implements a TCP ping client that:
    - Establishes TCP connections to remote servers
    - Sends TCP ping messages and measures round-trip time (RTT)
    - Handles connection-related errors (reset, unreachable, timeouts)
    - Calculates comprehensive network statistics
    - Reports packet loss and connection failures
    - Provides detailed performance metrics (min, max, average RTT)

USE CASES:
    - TCP connection establishment and performance testing
    - Network connectivity verification over TCP
    - Latency measurement for TCP-based services
